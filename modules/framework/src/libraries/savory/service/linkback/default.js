 ...
 */
Savory.Linkback = Savory.Linkback || function() {
	/** @exports Public as Savory.Linkback */
    var Public = {}
	 ...
	 * <p>
	 * To work properly, {@link Savory.RPC#settings} should be called <i>after</i> calling this.
	 * <p>
	 * Can only be called from Prudence configuration scripts!
	 */
	Public.settings = function() {
		resourcesPassThrough.push('/savory/service/linkback/trackback/')
		...
		modules.push(function() {
			...
			Savory.RPC.exportMethods({
				module: 'Pingback',
				namespace: 'pingback',
				object: 'Savory.Linkback.Pingback',
				dependencies: '/savory/service/linkback/',
				reset: true
			})
			return null
		})

		routes = predefinedGlobals['savory.service.rpc.routes'] = (predefinedGlobals['savory.service.rpc.routes'] || {})
		routes['/pingback/'] = {
			module: 'Pingback',
			type: 'xml'
		}
	...
	Public.ping = function(params) {
		var client = new Savory.RPC.Client({
	...
	Public.discover = function(uri, tryPingbackHeaders, tryPingbackContent, tryTrackback) {
		...
		if (result) {
			...
			if (tryPingbackContent) {
				var doc = Savory.HTML.parse(result.representation)
	...
	/**
	 * This namespace is exported via {@link Savory.RPC} as XML-RPC.
	 * 
	 * @namespace
	 */
	Public.Pingback = {
		ping: function(sourceUri, targetUri) {
			...
			var doc = Savory.HTML.parse(html)
