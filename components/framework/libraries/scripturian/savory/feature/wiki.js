 ...
 * @version 1.0
 */
Savory.Wiki = Savory.Wiki || function() {
	/** @exports Public as Savory.Wiki */
    var Public = {}
	...
	Public.settings = function() {
		dynamicWebPassThrough.push('/savory/feature/wiki/page/')
	}
    ...
    Public.routing = function() {
		...
		router.captureAndHide('/page/{page}/', '/savory/feature/wiki/page/').template.variables.put('page', uriPathVariable)

		dynamicWeb = router.filterBase(dynamicWebBaseURL, '/savory/feature/wiki/page-filter/', applicationInstance.context, dynamicWeb).next
	},
	...
	Public.getPage = function(conversation) {
		var page = conversation.locals.get('savory.feature.wiki.page')
		if (!page) {
			var name = conversation.locals.get('page')
			if (name) {
				var page = Public.getPageByName(name, conversation.query.get('revision'))
				if (page) {
					conversation.locals.put('savory.feature.wiki.page', page)
				}
			}
		}
	...
	Public.Form = Sincerity.Classes.define(function(Module) {
		...
		Public.render = function() {
			...
			var permissions = this.conversation.locals.get('savory.service.authorization.permissions')
			
			if ((mode == 'edit') && (!permissions || !permissions.get('savory.feature.pages.edit'))) {
				mode = 'view'
			}
			
			if ((mode == 'edit') && page && (String(this.conversation.request.method) == 'POST')) {
				var source = this.conversation.form.get('source')
				var session = this.conversation.locals.get('savory.service.authentication.session')
			...
			this.document.include('/savory/feature/wiki/page/' + mode + '/')
		}
