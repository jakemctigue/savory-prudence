 ...
 * @version 1.0
 */
Savory.Facebook = Savory.Facebook || function() {
	/** @exports Public as Savory.Facebook */
    var Public = {}
	 ...
	 * 
	 * @class
	 * @name Savory.Facebook.Application
	 * 
	 * @param [appId=application.globals.get('savory.integration.backend.facebook.appId')]
	...
	Public.Application = Sincerity.Classes.define(function() {
		 ...
		 * 
		 * @param conversation The Prudence conversation 
		 * @returns {Savory.Facebook.Session}
		 */
		Public.getSession = function(conversation) {
			...
			if (signedPayload) {
				if (signedPayload.oauth_token) {
					...
					return new Savory.Facebook.Session(this, signedPayload.oauth_token)
				}
			}
			...
			if (payload.code) {
				if (!Savory.Nonces.check(payload.state)) {
					return null
				}

				var accessToken = this.getAccessToken(payload.code, conversation.query.get('from'))
				if (accessToken) {
					return new Savory.Facebook.Session(this, accessToken)
				}
			}
	...
	Public.Session = Sincerity.Classes.define(function() {
		...
		/**
		 * Shortcut to {@link Savory.Facebook.Application#getGraphJson}.
		 */
